...
class DatabaseUtill() {
    ...
    fun sendLedgerRequestToFriend(
        friendUID: String,
        ledger: SingleLedgers,
        callback: OnUserDetailUpdate,
    ) {


        db_reference.child("users").child(friendUID).child("user_single_Ledgers").get()
            .addOnCompleteListener {
                ...
                if (it.result.exists()) {
                    ...
                    db_reference.child("users").child(friendUID).child("user_single_Ledgers")
                        .setValue(arrayList).addOnCompleteListener {
                            if (it.isSuccessful) {
                    ...
                    Log.d(TAG,
                        "sendLedgerRequestToFriend: Friend Request Failure -> ${it.exception}")
                    var list = ArrayList<SingleLedgers>()
                    list.add(ledger)
                    db_reference.child("users").child(friendUID).child("user_single_Ledgers")
                        .setValue(list).addOnCompleteListener {
                            if (it.isSuccessful) {
                                ...
                                Log.d(TAG,
                                    "sendLedgerRequestToFriend: Friend Request failed completely! ")
...
//    }
    ...
    fun updateCurrentUser(uid: String, callback: OnUserDetailUpdate): User {
        ...
        var user: User = User
        db_reference.child("users").child(user.userID!!)
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
...
//    }
    ...
    fun createNewSingleLedger(
        uid: String, newLedger: SingleLedgers, newLedgerList: ArrayList<SingleLedgers>,
        ...
        db_reference.child("ledgerInfo").child(newLedger.ledgerUID.toString()).setValue(map)
            .addOnCompleteListener {
                if (it.isSuccessful) {
                    ...
                    db_reference.child("ledgerInfo").child(newLedger.ledgerUID.toString())
                        .child("ledger_Created_timeStamp").get().addOnCompleteListener {
                            ...
                            if (it.isSuccessful) {
                                ...
                                db_reference.child("users").child(uid).child("user_single_Ledgers")
                                    .setValue(newLedgerList).addOnCompleteListener {
                                        if (it.isSuccessful) {
                                ...
                                Log.d(TAG,
                                    "createNewSingleLedger: Could Not Fetch CreatedTimeStamp and Update Ledger")
                                callback.onSingleLedgerUpdated(false)
                            }
                        }
    ...
    private fun getMetaDataMapForLedger(newLedger: SingleLedgers): HashMap<String, Any> {
        ...
        map["total_amount"] = newLedger.total_amount!!
        map["total_entries"] = newLedger.total_entries!!
        map["give_take_flag"] = newLedger.give_take_flag!!
        map["ledgerCreatedByUID"] = newLedger.ledgerCreatedByUID!!
        map["ledger_Created_timeStamp"] = ServerValue.TIMESTAMP
    ...
    fun updateUserTotalLedgerCount(uid: String, callback: OnUserDetailUpdate) {
        ...
        db_reference.child("users").child(uid).child("total_single_ledgers")
            .setValue(User.getUserSingleLedgers()?.size).addOnCompleteListener {
                if (it.isSuccessful) {
    ...
    private fun removeLedgerFromFirebase(newledger: SingleLedgers) {
        db_reference.child("ledgerEntries").child(newledger.ledgerUID.toString()).setValue(null)
            .addOnCompleteListener {
                if (it.isSuccessful) {
    ...
    private fun removeLedgerFromFriend(newledger: SingleLedgers) {
        ...
        db_reference.child("users").child(newledger.friend_userID!!).child("user_single_Ledgers")
            .get().addOnCompleteListener {
                if (it.result.exists()) {
                    ...
                    db_reference.child("users").child(newledger.friend_userID!!)
                        .child("user_single_Ledgers").setValue(arrayList).addOnCompleteListener {
                            if (it.isSuccessful) {
    ...
    // this is for total Gave/Give which will be shown on main Single Ledger Screen


    fun singleLedgersTotalMetaData():Pair<Float,Boolean>{
        var amount=0f
        var flag = false
        if (User.getUserSingleLedgers() !=null){
            var gaveAmount=0f
            ...
            for (ledger in User.getUserSingleLedgers()!!){

                if (ledger.give_take_flag == GAVE_ENTRY_FLAG){
                    gaveAmount += ledger.total_amount!!
                }
                if (ledger.give_take_flag == GET_ENTRY_FLAG){
                    getAmount+= ledger.total_amount!!
                }
            }

            if (gaveAmount>getAmount){

                amount=gaveAmount-getAmount
                flag= GAVE_ENTRY_FLAG
            }
            if (getAmount>gaveAmount){
                amount= getAmount-gaveAmount
                flag= GET_ENTRY_FLAG
